...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  for (int i=0; i<N-rem; i+=VECTOR_WIDTH) {
  ...
  if (rem > 0) {
    ...
    int loopend = N-rem;
    maskAll  = _cs149_init_ones(rem);
    result = _cs149_vset_float(1.f);

    _cs149_vload_float(x, values+loopend, maskAll); // Load all x vals
    _cs149_vload_int(y, exponents+loopend, maskAll); // Load all y vals
    ...
    // Write results back to memory
    _cs149_vstore_float(output+loopend, result, maskAll);
  }
...
float arraySumVector(float* values, int N) {
  ...
  //
  __cs149_mask maskAll = _cs149_init_ones(VECTOR_WIDTH);
  __cs149_vec<float> sum;
  _cs149_vset_float(sum, 0.f, maskAll); // init sum w all 0s
  ...
  _cs149_hadd_float(sum, sum);
  float sumarr[VECTOR_WIDTH];
  _cs149_vstore_float(sumarr, sum, maskAll);
  return sumarr[0];
}
